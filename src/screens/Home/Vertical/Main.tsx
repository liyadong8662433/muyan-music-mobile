...
const PlaylistPage = () => {
  const [visible, setVisible] = useState(commonState.navActiveId == 'nav_playlist')
  const component = useMemo(() => <Playlist />, [])
  useEffect(() => {
    let currentId: CommonState['navActiveId'] = commonState.navActiveId
    const handleNavIdUpdate = (id: CommonState['navActiveId']) => {
      currentId = id
...
const Main = () => {
  ...
  const component = useMemo(() => (
    ...
    >
      ...
      <View collapsable={false} key="nav_playlist" style={styles.pageStyle}>
        <PlaylistPage />
      </View>
